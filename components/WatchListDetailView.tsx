...
export default function WatchListDetailView({
  ...
  const loadDetail = async () => {
    try {
      ...
      const targetSlug = json.list?.slug || initialSlug;
      const targetId = json.list?.id || initialListId;
  ...
  const mainContent = (
    <div className="space-y-8 max-w-6xl mx-auto w-full flex-1">
      ...
      <div className="glass-panel p-6 sm:p-7 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 shadow-xs relative overflow-hidden space-y-4">
        {/* Subtle background glow accent */}
        <div className="absolute top-0 right-0 w-96 h-96 bg-blue-500/5 dark:bg-blue-500/10 rounded-full blur-3xl pointer-events-none -z-10" />
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          {/* Right Column: Sleek Grouped Action Toolbar */}
          <div className="flex items-center gap-2 flex-wrap shrink-0 lg:self-center">
            {isAdmin && (
              <div className="flex items-center gap-1.5 p-1 rounded-xl bg-rose-500/5 dark:bg-rose-500/10 border border-rose-500/20">
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 text-white text-[11px] font-semibold flex items-center gap-1 shadow-xs transition-all cursor-pointer"
                ...
                {data?.list?.userId && data.list.userId !== currentUserId && (
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-rose-500/30 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 text-[11px] font-semibold flex items-center gap-1 transition-all cursor-pointer"
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer shadow-xs ${
                following ? 'bg-emerald-600 hover:bg-emerald-700 text-white border border-emerald-500' : 'bg-blue-600 hover:bg-blue-700 text-white'
              }`}
            >
              <Bell className="w-3.5 h-3.5" />
              {following ? 'Alerts Active' : 'Follow & Get Alerts'}
            </button>
            ...
            {isMaintainer ? (
              <>
                <button
                  type="button"
                  onClick={() => setShowEditListModal(true)}
                  className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs font-semibold flex items-center gap-1.5 cursor-pointer transition-all shadow-xs"
                  title="Edit Watch List Name & Details"
                >
              ...
              <>
                ...
                <button
                  type="button"
                  onClick={() => (!authUser ? setShowAuthRequiredModal(true) : handleForkList())}
                  disabled={forking}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-1 sticky top-[72px] h-[calc(100vh-88px)] flex flex-col glass-panel rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden z-20">
                        ...
                        {isMaintainer && (
                          <div className="relative shrink-0">
                            ...
                            {openMenuId === p.id && (
                              <div
                                ref={menuRef}
                                className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-30 p-1.5 space-y-0.5 animate-in fade-in zoom-in-95 duration-100"
                              >
                                <button
                                  type="button"
        ...
        <div className="lg:col-span-2 sticky top-[72px] h-[calc(100vh-88px)] flex flex-col glass-panel rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden z-20">
          {/* Sticky Column Header Bar */}
